...
| `useWhitelistFile`          | Use `whitelist.json` instead of the `whiteListedNames` property. Checks both username and UUID                                                                                                                                           | false                                           |
| `useBlacklistFiles`         | Use `banned-ips.json` and `banned-players.json` to prevent players from waking up the server. Can be used together with `blackListedAddress` property                                                                                    | false                                           |
